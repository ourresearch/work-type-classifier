...
| **I5** | **final hybrid** | **TEST (locked)** | **0.763** | **0.527** | **0.820 / 0.929** | **ship** |

**Bottom line:** the hybrid beats the keep-oa_type baseline on accuracy (0.763 vs 0.716) with no
overfit (train/val gap ≈ 0), recovers the biggest OpenAlex errors — conference-paper recall 0→0.73,
conference-abstract 0→0.49, paratext 0→0.37 — while **article precision stays 0.82** (article is not
sacrificed). Editorial (~0.19) remains the hard article-boundary residual. The de-leaked LR edges the
hybrid on macro-F1 but is not deterministic/SQL-deployable; the hybrid is, per the team's preference.
...
- **Result:** acc=0.763, macro-F1=0.527, article P=0.820/R=0.929
