# OpenAlex work types — canonical list & definitions

The canonical OpenAlex work `type` vocabulary: **25 types**, each with an annotator-grade definition.
This is the contract the work-type classifier in this repo classifies toward, and the source for the
public `/types` API descriptions. Decided by OpenAlex at the 2026-06-29/30 offsite.

Each entry is a **definition** paragraph (concept only), then **Includes** (examples) and
**Goes elsewhere** (common misfiles).

**The 25 types:** `article`, `book`, `book-chapter`, `book-review`, `conference-abstract`,
`conference-paper`, `data-paper`, `dataset`, `dissertation`, `editorial`, `erratum`, `letter`,
`libguides`, `other`, `paratext`, `peer-review`, `preprint`, `reference-entry`, `report`,
`retraction`, `review`, `software-paper`, `software`, `standard`, `supplementary-materials`.

**Source trust order** (where sources conflict, higher wins): (1) OpenAlex annotator notes
(authoritative); (2) Web of Science document-type descriptions; (3) Scopus; (4) the legacy OpenAlex
API `type` text — being replaced.
...
- **`software-paper` / `software` definitions** — WoS has no equivalent document type (only *Software Review*, an appraisal). Currently modeled on `data-paper` / `dataset`; wording to confirm.
- **`review` operational rule** — adopt WoS's "must have references + the title claim must recur in the body" as the definition, or keep OpenAlex's venue-derived rule? (A classifier question.)
